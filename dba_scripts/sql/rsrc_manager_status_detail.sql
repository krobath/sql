...
SELECT se.sid sess_id, s.username, s.osuser, s.machine, s.module, s.program, co.name consumer_group, 
 ...
 AND SE.SID = s.sid
 --AND s.username LIKE 'NC_%'
 --AND s.SERVICE_NAME <>'SYS$BACKGROUND'
 AND co.name = 'OLTP_GROUP'
 ORDER BY se.queued_time DESC;